...
static void calculate_avg_output_time(double start_output_picture2,
...
cv::Point rectCenter(cv::Point point1, cv::Point point2);
...
// [x, y, a, frame#]
double foundPoints[2][4] = { { 0, 0, 0, 0 }, { 0, 0, 0, 0 } };
...
cv::Mat myViewMat;
cv::Mat* mat;
cv::Mat img2, prevGray;

jobject gAddrDescriptor;

const int history = 2;
std::vector<cv::Point2f> points[history];
...
static double get_audio_clock(VideoState *is) {
	...
	if (is->audio_st) {
		bytes_per_sec = is->audio_st->codec->sample_rate * 2
...
void fill_rectangle(SDL_Surface *screen, int x, int y, int w, int h,
		int color) {
	SDL_Rect rect;
	rect.x = x;
	rect.y = y;
...
static void stream_seek_special(VideoState *is, int64_t pos, int64_t rel,
		int seek_by_bytes) {
	if (!is->seek_req) {
		is->seek_pos = pos;
		is->seek_rel = rel;
		...
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"Seek Requested:%d", g_seek_duration);
	}
} /* stream_seek() */
...
static void broov_set_current_duration(ULONG secs) {
	...
	if (secs >= 356400) {
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"Unexpected CurrentDuration:%ld ", secs);
		g_current_duration = 0;
		return;
	}

	if (g_seek_success) {
		if ((secs == g_current_duration) || (secs == g_current_duration - 1)
			...
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"Skipping Set Current Duration:%ld ", secs);
			//Fail safe mechanism
			reset_counter++;
			if (reset_counter >= 5) {
				g_seek_success = 0;
...
static int try_to_set_best_video_mode(int w, int h, int rgb565) {
	...
	if (vi) {
		...
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"FullScreen Width:%d Height:%d", fs_screen_width,
				fs_screen_height);
	...
	if (!vp->display_rgb_text) {
		if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB8888) {
			...
			if (g_source_width_height) {
				vp->display_rgb_text = SDL_CreateTexture(
						SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC,
						vp->width, vp->height);
			} else {
				vp->display_rgb_text = SDL_CreateTexture(
						SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC,
						vp->dst_width, vp->dst_height);
			}

		} else {

			if (g_source_width_height) {
				vp->display_rgb_text = SDL_CreateTexture(SDL_PIXELFORMAT_RGB565,
						SDL_TEXTUREACCESS_STATIC, vp->width, vp->height);
			} else {
				vp->display_rgb_text = SDL_CreateTexture(SDL_PIXELFORMAT_RGB565,
						SDL_TEXTUREACCESS_STATIC, vp->dst_width,
						vp->dst_height);
			}
		}
		SDL_UpdateTexture(vp->display_rgb_text, NULL, vp->pFrameRGB->data[0],
...
static void alloc_rgb_picture(void *userdata) {
	...
	if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB8888) {
		...
		if (g_source_width_height) {
			vp->numBytes = avpicture_get_size(PIX_FMT_RGBA, vp->width,
					vp->height);
		} else {
		...
		if (g_source_width_height) {
			vp->numBytes = avpicture_get_size(PIX_FMT_RGB565, vp->width,
					vp->height);
		} else {
		...
		if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB8888) {
			if (g_source_width_height) {
				avpicture_fill((AVPicture*) vp->pFrameRGB, vp->buffer,
						PIX_FMT_RGBA, vp->width, vp->height);
			} else {
				avpicture_fill((AVPicture*) vp->pFrameRGB, vp->buffer,
			...
			if (g_source_width_height) {
				avpicture_fill((AVPicture*) vp->pFrameRGB, vp->buffer,
						PIX_FMT_RGB565, vp->width, vp->height);
			} else {
				avpicture_fill((AVPicture*) vp->pFrameRGB, vp->buffer,
						PIX_FMT_RGB565, width, height);
			}
		}
...
cv::Point coord2grid(cv::Point currentPoint, cv::Point gridCenter,
		int gridSectionLength) {
	int retX = gridCenter.x;
	int retY = gridCenter.y;

	retX = gridCenter.x + (gridSectionLength * currentPoint.x);

	retY = gridCenter.y + (gridSectionLength * currentPoint.y);

	return cv::Point(retX, retY);
}

cv::Point grid2coord(cv::Point currentPoint, cv::Point gridCenter,
		int gridSectionLength) {
	int retX = gridCenter.x;
	int retY = gridCenter.y;

	retX = ((currentPoint.x - gridCenter.x) / gridSectionLength);

	retY = ((currentPoint.y - gridCenter.y) / gridSectionLength);
 ...
 */
static int rgb_queue_picture(VideoState *is, AVFrame *pFrame, double pts,
		int64_t pos) {
	VideoPicture *vp;
	...
	if ((g_source_width_height) && (!vp->pFrameRGB)) {
		...
		if (!vp->pFrameRGB || vp->dst_width != g_aspect_ratio_w
				|| vp->dst_height != g_aspect_ratio_h) {
			...
			is->img_convert_ctx = sws_getCachedContext(is->img_convert_ctx,
					...
					vp->dst_height, dst_pix_fmt, sws_flags, NULL, NULL, NULL);
	...
	if (vp->pFrameRGB) {
		...
		if (g_asm_yuv2rgb) {
			...
			if (g_video_yuv_rgb_asm == 0) {
				sws_scale(is->img_convert_ctx, pFrame->data, pFrame->linesize,
						0, vp->height, vp->pFrameRGB->data,
						vp->pFrameRGB->linesize);
			} else {
				if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB565) {

					if (g_source_width_height) {
						yuv420_2_rgb565(vp->pFrameRGB->data[0], pFrame->data[0],
						...
						yuv420_2_rgb565(vp->pFrameRGB->data[0], pFrame->data[0],
								pFrame->data[1], pFrame->data[2], vp->dst_width,
								vp->dst_height, pFrame->linesize[0], // Y span
								pFrame->linesize[1], // UV span Width / 2
					...
					if (g_source_width_height) {
						yuv420_2_rgb8888(vp->pFrameRGB->data[0],
								pFrame->data[0], pFrame->data[2],
								pFrame->data[1], is->video_st->codec->width,
								is->video_st->codec->height,
								pFrame->linesize[0], // Y span
								pFrame->linesize[1], // UV span Width / 2
			...
			sws_scale(is->img_convert_ctx, pFrame->data, pFrame->linesize, 0,
					vp->height, vp->pFrameRGB->data, vp->pFrameRGB->linesize);
		}
...
cv::Point rectCenter(cv::Point point1, cv::Point point2) {
	cv::Point result = cv::Point((point1.x + point2.x) / 2,
			(point1.y + point2.y) / 2);
...
bool pointIn(cv::Point2f point, double radius) {
	...
	for (h = history - 1; h > 1; h--) {
		if (found)
			break;

		if (!points[h].empty()) {
			for (k = 0; k < points[h].size(); k++) {
				cv::Rect region(
						cv::Point(points[h][k].x - radius,
								points[h][k].y - radius),
						cv::Point(points[h][k].x + radius,
								points[h][k].y + radius));

				if (point.inside(region)) {
					found = true;
					break;
				}
			}
...
void player_mat(long addrDescriptor, JNIEnv *env, jobject obj) {
	...
	parentObj = env->NewGlobalRef(obj);
...
static int video_module_init(VideoState *is) {
	...
	__android_log_print(ANDROID_LOG_INFO, "AV Skip", "secondsperFrame: %lf",
			secondsPerFrame);
...
static int stream_component_open(VideoState *is, int stream_index) {
	...
	if (!codec) {
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"avcodec_find_decoder failed %d Name:%s\n", avctx->codec_id,
				avctx->codec_name);
		return -1;
	}
...
static void stream_component_close(VideoState *is, int stream_index) {
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"Wait for Video Thread Over");
		packet_queue_end(&is->videoq);
#endif
		__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
				"Stream Component Close Video Done");
		break;
...
static void broov_update_aspect_ratio() {
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"Inside Update Aspect Ratio X:%d Y:%d W:%d H:%d", g_aspect_ratio_x,
			g_aspect_ratio_y, g_aspect_ratio_w, g_aspect_ratio_h);
	...
	if (cur_stream->video_st->codec->sample_aspect_ratio.num == 0) {
		...
		aspect_ratio = av_q2d(cur_stream->video_st->codec->sample_aspect_ratio)
				* cur_stream->video_st->codec->width
				/ cur_stream->video_st->codec->height;
	}
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"Updated Aspect Ratio X:%d Y:%d W:%d H:%d", g_aspect_ratio_x,
			g_aspect_ratio_y, g_aspect_ratio_w, g_aspect_ratio_h);
}
...
static int decode_module_init(void *arg) {
	...
	if (start_time != AV_NOPTS_VALUE) {
		...
		ret = avformat_seek_file(pFormatCtx, -1, INT64_MIN, timestamp,
				INT64_MAX, 0);
	...
	if (audio_index >= 0) {
		...
		if (stream_component_open(is, audio_index) < 0) {
			...
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"Audio index:%d failed, finding next stream", audio_index);
			for (i = 0; i < pFormatCtx->nb_streams; i++) {
				if (pFormatCtx->streams[i]->codec->codec_type
						== AVMEDIA_TYPE_AUDIO) {
					audio_index = i;
					__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
							"Audio index:%d", audio_index);
					if (stream_component_open(is, audio_index) == 0) {
						__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
								"Audio index:%d Successful", audio_index);
						break;
					}
				}
...
int player_init(char *font_fname, int subtitle_show, int subtitle_font_size,
		int subtitle_encoding_type, int rgb565) {
...

	{
		...
		if (font_init == 0) {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"BroovFont init success");

		} else {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"BroovFont init failed");
		}
	}

	{
		...
		if (initted & img_flags != img_flags) {
			__android_log_print(ANDROID_LOG_INFO, "libSDL",
					"SDL_Image Initialization failure");
			return (4);
		}
...
int process_a_ui_event(VideoState *is) {
...

	{
		...
		case SDL_KEYDOWN:
			...
			case SDLK_x:
				audio_clock_delta +=
						(event.key.keysym.sym == SDLK_z) ? -0.1 : 0.1;
				__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
						"Audio sync correction : %+.1fs", audio_clock_delta);
				break;
			...
			case SDLK_RIGHT:
				...
				if (cur_stream) {
					if (seek_by_bytes) {
						if (cur_stream->video_st >= 0
								&& cur_stream->video_current_pos >= 0) {
							pos = cur_stream->video_current_pos;
						} else if (cur_stream->audio_st >= 0
		...
		case SDL_VIDEORESIZE: {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"VideoResize event received W:%d, H:%d", event.resize.w,
					event.resize.h);
			if (g_video_output_rgb_type == VIDEO_OUTPUT_RGB8888) {
		...
		case FF_GUI_ASPECT_RATIO_EVENT: {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"Aspect Ratio Received:%d", event.user.data1);
			{
		...
		case FF_GUI_SEEK_EVENT: {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
					"Seek Value Received:%d", event.user.data1);
			if (g_total_duration) {
				int percent = (int) event.user.data1;
				double current_percentile = (percent / 10.0);
				double per_percentile_num_of_secs = (double) (g_total_duration
						/ 100.0);
				incr =
						(double) (current_percentile
								* per_percentile_num_of_secs);
				__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
						"CurrentPercentileSeek:%lf PerPercentileSeek:%lf, DurationToMove:%lf",
						current_percentile, per_percentile_num_of_secs, incr);
		...
		case SDL_MOUSEBUTTONDOWN: {
			...
			do_seek_special: __android_log_print(ANDROID_LOG_INFO,
					"BroovPlayer", "Do Seek Special");
			if (cur_stream) {
				...
				if (seek_by_bytes) {
					...
					stream_seek_special(cur_stream,
...
int decode_module_clean_up(void *arg) {
	VideoState *is = (VideoState *) arg;
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"DecodeModule CleanUp");

	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"Waiting for DecodeThread Exit");
	SDL_WaitThread(is->parse_tid, NULL);
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"Waiting for DecodeThread Over");
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"DecodeModule After Audio Stream Close");

	if (is->videoStream >= 0)
		stream_component_close(is, is->videoStream);

	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"DecodeModule After Video Stream Close");
...
static int video_thread(void *arg) {
	...
	for (;;) {
...

		{
			...
			avcodec_decode_video2(is->video_st->codec, g_video_frame,
				...
				if (g_skip_frames) {
					if (skip_now(is)) {
...

						{
							...
							frame_delay = av_q2d(
									is->video_st->codec->time_base);

							/* for MPEG2, the frame can be repeated, so we update the
							 clock accordingly */
							frame_delay += g_video_frame->repeat_pict
									* (frame_delay * 0.5);
							is->video_clock += frame_delay;

							rgb_queue_picture(is, g_video_frame, pts,
									packet->pos);
						...
						calculate_avg_output_time(start_output_picture2,
								end_output_picture2);
					} else {
						skipped = true;
					}
	...
	while (1) {
		...
		if (is->seek_req) {
			if (is->seek_req_special) {
				...
				ret = avformat_seek_file(is->pFormatCtx, -1, seek_min,
		...
		if (g_eof) {
			...
			if (is->audioStream >= 0&&
			is->audio_st->codec->codec->capabilities & CODEC_CAP_DELAY) {av_init_packet(packet);
			packet->data = NULL;
			packet->size = 0;
...
static void broov_init_global_values(int loop_after_play, int audio_file_type,
		...
		int audio_queue_size, int fast_mode, int debug_mode, int sync_type,
		int seek_duration, int ffmpeg_flags) {
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer",
			"Loop:%d FileType:%d SkipFrames:%d RGB565:%d YUV_RGB_ASM:%d",
			loop_after_play, audio_file_type, skip_frames, rgb565, yuv_rgb_asm);
...
#else
	if (yuv_rgb_asm == 1) {
		g_source_width_height = 1;
		g_video_yuv_rgb_asm = 1;
	} else {
		g_source_width_height = 0;
		g_video_yuv_rgb_asm = 0;
	}
#endif

	if (skip_bidir_frames) {
		g_last_skip_type = AVDISCARD_BIDIR;
	} else {
		g_last_skip_type = AVDISCARD_DEFAULT;
	}
	...
	if (vqueue_size_min) {
		BROOV_VIDEO_MIN_BUFFER_SIZE = vqueue_size_min;
	}
	if (vqueue_size_max) {
		BROOV_VIDEO_MAX_BUFFER_SIZE = vqueue_size_max;
	}
	if (total_queue_size) {
		BROOV_TOTAL_MAX_BUFFER_SIZE = total_queue_size;
	}
	if (audio_queue_size) {
		MAX_AUDIOQ_SIZE = audio_queue_size;
	}
...
int player_main(int argc, char *argv[], int loop_after_play,
		int audio_file_type, int skip_frames, int rgb565, int yuv_rgb_asm,
		int skip_bidir_frames, int vqueue_size_min, int vqueue_size_max,
		int total_queue_size, int audio_queue_size, int fast_mode,
		int debug_mode, int sync_type, int seek_duration, int ffmpeg_flags)
	...
	broov_init_global_values(loop_after_play, audio_file_type, skip_frames,
			rgb565, yuv_rgb_asm, skip_bidir_frames, vqueue_size_min,
			vqueue_size_max, total_queue_size, audio_queue_size, fast_mode,
			debug_mode, sync_type, seek_duration, ffmpeg_flags);
	...
	while (1) {
		...
		//if ((ui_scheduler%64) == 0)
		{
			...
			if (ret_value == BROOV_FF_QUIT || ret_value == BROOV_SDL_QUIT) {
				break;
			}
		}
		//ui_scheduler++;
...
static void calculate_avg_output_time(double start_output_picture2,
	...
	output_picture_time = output_picture_time + end_output_picture2
			- start_output_picture2;
	...
	if (average_number_for_output_time >= 5) {
		avg_outputtime_missed = output_picture_time
				/ average_number_for_output_time;
...
static double calculate_file_fps(VideoState *is) {
	...
	if (is->video_st->codec->time_base.den
			!= rfps * is->video_st->codec->ticks_per_frame
			|| is->video_st->codec->time_base.num != rfps_base) {
		file_fps_in = ((float) rfps / rfps_base);
		__android_log_print(ANDROID_LOG_INFO, "AV Skip", "codec rfps : %lf",
				file_fps);
	}
	return file_fps_in;
}
...
static double calculate_video_lag(double videoTime, double systemMasterClock) {
	__android_log_print(ANDROID_LOG_INFO, "AV Skip",
			"vlref_clock : %lf, systemMasterClock : %lf", vlref_clock,
			systemMasterClock);
	double actualtime_fromstart = (systemMasterClock - vlref_clock);
	double videotimelag = actualtime_fromstart - videoTime;
	__android_log_print(ANDROID_LOG_INFO, "AV Skip",
			"time_fromstart : %lf, videotimelag : %lf", actualtime_fromstart,
			videotimelag);
	double video_fps_lag = videotimelag * file_fps;
	__android_log_print(ANDROID_LOG_INFO, "AV Skip", "video_fps_lag : %lf",
			video_fps_lag);
	video_lag_err = (video_fps_lag);
	__android_log_print(ANDROID_LOG_INFO, "AV Skip",
			"video_fps_lag per second : %lf", (video_lag_err / 60));
}
