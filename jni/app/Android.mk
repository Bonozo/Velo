...
OPENCV_SRC:=/home/chimwemwe/android-workspace/AppVelo/native/opencv2.4.3.2
include $(OPENCV_SRC)/native/jni/OpenCV.mk
...
ifeq ($(TARGET_ARCH_ABI),x86)
   ...
   LOCAL_LDLIBS += $(OPENCV_SRC)/"native/libs/x86/libopencv_java.so"
else
ifeq ($(TARGET_ARCH_ABI),mips)
   LOCAL_LDLIBS += $(AVPLAYER_PATH)/"native/ffmpeg/ffmpeg-0.11.1/android/mips/libffmpeg.so"
   LOCAL_LDLIBS += $(OPENCV_SRC)/"native/libs/mips/libopencv_java.so"
else
   LOCAL_LDLIBS += $(AVPLAYER_PATH)/"native/ffmpeg/ffmpeg-0.11.1/android/armv6_vfp/libffmpeg.so"
   LOCAL_LDLIBS += $(OPENCV_SRC)/"native/libs/armeabi-v7a/libopencv_java.so"
